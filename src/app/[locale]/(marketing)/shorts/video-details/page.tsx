...
export default function VideoDetailsPage() {
  ...
  const keyTopics = t.raw('topics') as string[];
  const resourcesList = t.raw('resourcesList') as string[];
  const relatedVideos = (tShorts.raw('videos') as RelatedVideo[]).map((video, i) => ({
    ...video,
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-8 pb-16">
        <div className="flex flex-col gap-[50px] w-full">
          <Reveal delay={0.1} direction="up">
            <div className="relative w-full aspect-video rounded-[20px] overflow-hidden flex items-center justify-center cursor-pointer group"
                  ...
                  <motion.div className="flex flex-col gap-3 mt-2" variants={containerVariants}>
                    {resourcesList.map((label, i) => (
                      <motion.div key={i} variants={itemVariants}>
                        <Link href="#" className="flex items-center justify-between w-full h-[58px] rounded-[12px] border border-[#E8CFC1] bg-white px-4 hover:border-[#781E36] transition-colors">
            ...
            <div className="flex flex-col gap-6 w-full">
              ...
              >
                {relatedVideos.map((card, i) => (
                  <motion.div key={i} variants={itemVariants}>
                    <Link href="/shorts/video-details" className="flex flex-col w-full rounded-[20px] border border-[#E8CFC1] bg-white overflow-hidden hover:shadow-lg transition-shadow"
                      style={{ boxShadow: '0px 1px 2px -1px #0000001A, 0px 1px 3px 0px #0000001A' }}>
